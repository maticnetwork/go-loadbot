...
func main() {
	...
	balance, err = cl.BalanceAt(ctx, accs[0].Address, nil)
	if err != nil {
		log.Println("Error in checking balance: ", err)
	}
	fmt.Println("Balance1: ", balance)

	nonce, err := cl.PendingNonceAt(ctx, accs[0].Address)
	if err != nil {
	...
	fundAccounts(ctx, cl, generatedAccounts, chainID, accs[0], ksOpts)
...
func fundAccounts(ctx context.Context, client *ethclient.Client, genAccounts Accounts, chainID *big.Int,
	sender accounts.Account, opts *bind.TransactOpts) {
	for i := 0; i < N; i++ {
		runTransaction(ctx, client, genAccounts[i].addr, chainID, sender, opts, Nonce+uint64(i), 1000000000000000000)
	}
}

func runTransaction(ctx context.Context, Clients *ethclient.Client, recipient common.Address, chainID *big.Int,
	sender accounts.Account, opts *bind.TransactOpts, nonce uint64, value int64) {
	...
	signedTx, err := opts.Signer(sender.Address, tx)
